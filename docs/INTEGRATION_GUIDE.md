...
Configure your affiliate network to send conversion postbacks:
...
**Required Parameters:**
- `click_id` - Unique click identifier (passed automatically via `{click_id}` macro)
- `payout` - Commission amount for the conversion
- `status` - Conversion status (approved, pending, rejected)
- `security_token` - Your secret security token from Settings

**Example Postback URL:**
```
https://rdajybqalmsdycxsruon.supabase.co/functions/v1/postback?click_id={click_id}&payout=25.50&status=approved&security_token=your_secret_token
```
...
**MaxBounty:**
```
https://rdajybqalmsdycxsruon.supabase.co/functions/v1/postback?click_id=[subid]&payout=[payout]&status=approved&security_token=YOUR_TOKEN
```

**ClickBank:**
```
https://rdajybqalmsdycxsruon.supabase.co/functions/v1/postback?click_id=[CLICKID]&payout=[AMOUNT]&status=approved&security_token=YOUR_TOKEN
```

**ShareASale:**
```
https://rdajybqalmsdycxsruon.supabase.co/functions/v1/postback?click_id=[MERCHANTDEFINEDTYPE]&payout=[AMOUNT]&status=approved&security_token=YOUR_TOKEN
```
...
✅ Security token matches (check in Settings → Profile)
...
2. Test postback manually with curl (see above)
3. Verify click_id exists in clicks table
4. Ensure security token hasn't been rotated
